...
문제의 조건 1~4를 순서대로 코드로 옮기는 구현 방법이 떠올랐다.

문제를 이해하기 어려웠다. 

- 벨트가 이동하면 내구도 배열도 이동하는가? (맞음.)
- 벨트가 이동하면 번호도 같이 이동하는가? (아님.)
- 1~4번이 각각 하나의 단계인지, 1~4를 통틀어 하나의 단계인지 알기 어려웠다. (이 문제의 경우 후자였다.)
<br>
<br>

# 🪄 코드 설계하기

1.로봇은 1번 칸에서 올라가서 N번칸에서 내린다. 따라서 로봇은 최대 n개
...
2번 조건에서 먼저 올라간 로봇부터 이동해야 함을 빠뜨렸다. for i in range(n)을 for i in range(n-2, -1, -1) 로 변경했다.

## 2회차

result =+ 1 을 result +=1로 문법 수정.
